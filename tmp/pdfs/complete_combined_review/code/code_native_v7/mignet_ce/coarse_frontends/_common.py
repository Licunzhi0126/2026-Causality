...
def _h5ad_obs_index(handle: h5py.File) -> list[str]:
    obs = handle["obs"]
    key = obs.attrs.get("_index", "_index")
    if isinstance(key, bytes):
        key = key.decode("utf-8")
    return _decode_h5(np.asarray(obs[str(key)]))


def _h5ad_coords(path: Path, units: list[str]) -> np.ndarray | None:
    with h5py.File(path, "r") as handle:
        obs_names = _h5ad_obs_index(handle)
        ...
        if "obsm/spatial" in handle:
            spatial = handle["obsm/spatial"]
            if isinstance(spatial, h5py.Group):
                raw_order = spatial.attrs.get("column-order", None)
                columns = ([x.decode("utf-8") if isinstance(x, bytes) else str(x) for x in raw_order]
                           if raw_order is not None else [k for k in spatial.keys() if k != "_index"])
                values = np.column_stack([np.asarray(spatial[c], dtype=np.float32) for c in columns])
            else:
                values = np.asarray(spatial, dtype=np.float32)
            return values[order, :2]
        obs = handle["obs"]
        if "x" in obs and "y" in obs:
            values = np.column_stack([np.asarray(obs["x"], dtype=np.float32), np.asarray(obs["y"], dtype=np.float32)])
            return values[order]
        return None
