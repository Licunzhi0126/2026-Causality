...
"""Differentiable native-V7 PIJ equations for WYT macro training.

This module mirrors the frozen NumPy V7 formula without substituting the true
GRN block G with H5AD Regulon/Module activity.  It is intentionally separate
from compare_NR_kl_sinkhorn_regsim_v7.py so provenance cannot confuse R and G.
"""
...
def pairwise_kl(source: torch.Tensor, target: torch.Tensor, *, beta: float) -> torch.Tensor:
...
def balanced_sinkhorn_from_cost(
    ...
    *,
    ...
    log_source = torch.full(
        (source_count,), -float(np.log(source_count)), dtype=cost.dtype, device=cost.device
    )
    log_target = torch.full(
        (target_count,), -float(np.log(target_count)), dtype=cost.dtype, device=cost.device
    )
    ...
    for _ in range(int(iterations)):
        source_potential = log_source - torch.logsumexp(
            log_kernel + target_potential[None, :], dim=1
        )
        target_potential = log_target - torch.logsumexp(
            log_kernel + source_potential[:, None], dim=0
        )
    joint = torch.exp(log_kernel + source_potential[:, None] + target_potential[None, :])
...
def native_v7_pij_torch(
    ...
    *,
    ...
    """Raw true-G KL + 0.25 robust-normalized N KL, then balanced Sinkhorn."""
