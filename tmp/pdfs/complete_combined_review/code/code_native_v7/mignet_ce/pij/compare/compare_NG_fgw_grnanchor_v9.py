...
def _select_pair_adjacencies(
    ...
    if context.feature_alignment_space != "native_units":
        raise ValueError("FGW V9 requires native-unit feature and graph alignment.")
    if side == "lower":
        ...
        raise ValueError("side must be one of ['lower', 'upper'].")
    if max(pair) >= len(graphs):
        raise ValueError(f"FGW V9 is missing {side} LightCCI graphs for pair {pair}.")

    matrices: list[sp.csr_matrix] = []
    for time_index in pair:
        graph = graphs[time_index]
        graph_units = list(map(str, graph.units))
        expected_units = list(map(str, units_by_time[time_index]))
        if graph_units != expected_units:
            raise ValueError(
                f"FGW V9 {side} graph units are not aligned at time index {time_index}."
            )
        stored = graph.metadata.get("adjacency_csr")
        if stored is None:
            raise ValueError(
                f"FGW V9 requires LightCCI adjacency_csr for {side} time index {time_index}."
            )
        ...
        if matrix.shape != (len(expected_units), len(expected_units)):
            raise ValueError(
                f"FGW V9 {side} adjacency shape {matrix.shape} does not match "
                f"{len(expected_units)} units at time index {time_index}."
            )
        matrices.append(matrix)
    return matrices[0], matrices[1]


class CompareNGFGWGRNAnchorV9PijMethod:
    """Frozen V5 node cost with fixed-iteration directed low-rank FGW."""

    name = "compare_NG_fgw_grnanchor_v9"
    feature_keys = ("N",)
    pij_key = "kl"
        ...
        metadata.update(
            {
                ...
                "algorithm_version": "lightcci_grn_lowrank_directed_fgw_v9",
    ...
    def _build_pair_kernel(
        ...
        *,
        ...
        joint, pij, fgw_metadata = solve_lowrank_directed_fgw(
            cost,
            source_adjacency,
            target_adjacency,
        )
        diagnostics = {
            "kind": "lowrank_directed_fgw_grnanchored_block_kl",
            "beta": FIXED_FEATURE_BETA,
    ...
    def run(
        ...
        kernels = TransitionKernels(
            kernel_metadata={
                ...
                "fusion_mode": "frozen_v5_node_cost_plus_lowrank_directed_fgw",
                "transition_construction": "fixed_iteration_lowrank_directed_fgw",
                "node_cost_source": "raw_GRN_KL_plus_0.25_robust_normalized_N_KL",
                "graph_cost_source": "current_pair_LightCCI_directed_adjacency",
                ...
                "fgw_factorization_seed": FGW_FACTORIZATION_SEED,
        ...
        for pair in pairs:
            ...
            for side, target_dict in (("lower", kernels.p_lower), ("upper", kernels.p_upper)):
                ...
                source_adjacency, target_adjacency = _select_pair_adjacencies(context, side, pair)
                grn_pairwise = (
        ...
        result = MethodResult(
            ...
            method_metadata={
                "pij_method": self.name,
                "representation": "lightcci_grn_lowrank_directed_fgw_v9",
                "compare_feature_keys": list(self.feature_keys),
                "compare_pij_method": self.pij_key,
                "fusion_mode": "frozen_v5_node_cost_plus_lowrank_directed_fgw",
                "transition_construction": "fixed_iteration_lowrank_directed_fgw",
