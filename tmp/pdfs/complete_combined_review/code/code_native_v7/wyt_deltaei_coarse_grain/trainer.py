...
class WYTDeltaEIConfig:
    ...
    def validate(self, prepared: PreparedCoarseInput) -> None:
        ...
        if self.local_graph_mode not in {"legacy_features", "coords", "all_features"}:
            raise ValueError("local_graph_mode must be one of legacy_features, coords, all_features.")
        if self.local_graph_mode == "coords" and (prepared.coords_t is None or prepared.coords_tp is None):
            raise ValueError("local_graph_mode=coords requires coordinates at both time points.")
...
def _torch_sparse(matrix: sp.spmatrix, device: torch.device) -> torch.Tensor:
    """Convert scipy matrices using a density-aware representation.

    True CCI_total matrices in this study are almost dense. Keeping them as
    torch sparse COO makes every S.T @ (A @ S) step dramatically slower on
    CPU while producing exactly the same values. Dense conversion is therefore
    used above 20% density; local kNN graphs remain sparse.
    """
...
def train_deltaei(
    ...
    encoder_preprocess_dims = config.local_dims if config.local_graph_mode == "legacy_features" else 0
