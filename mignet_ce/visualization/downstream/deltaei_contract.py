...
OPTIMIZED_MAPPINGS = (MAPPING_COMPLETE, MAPPING_MATURITY)
FORMAL_TIME_PAIRS = ("11.5->12.5", "12.5->13.5", "13.5->14.5")
FORMAL_K = 40
FORMAL_EPOCHS = 1500
FORMAL_NMF_MAX_ITER = 300
FORMAL_CACHE_PROTOCOL = "full_model_space_v2"
...
def deltaei_contract_row(record, *, tolerance: float = DELTAEI_CONTRACT_TOLERANCE) -> dict[str, object]:
    ...
    return {
        ...
        "hardK_t": int(np.count_nonzero(np.asarray(record.hard_s_full).sum(axis=0))),
        "hardK_tp": int(np.count_nonzero(np.asarray(record.hard_t_full).sum(axis=0))),
...
def audit_full_cache_root(
    ...
    *,
    ...
    table = pd.DataFrame(rows)
    if len(table) != 6:
        raise RuntimeError(f"Expected six optimized DeltaEI caches, found {len(table)} under {root}")
    return table
