...
def plot_cross_representation_consistency(consistency: pd.DataFrame, path: Path) -> None:
    ...
    for panel, (left, right) in enumerate(mapping_pairs):
        ...
        ax.plot(range(len(times)), subset["NMI"], "-o", color=UNIFIED_COLORS[left], label="NMI")
        ax.plot(range(len(times)), subset["ARI"], "--s", color=UNIFIED_COLORS[right], label="ARI")
    ...
    figure.suptitle(
        "All six pairwise cross-representation comparisons",
        color=NAVY,
        fontsize=16,
        weight="bold",
    )
