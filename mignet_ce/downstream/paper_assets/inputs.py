...
def load_vertical_metrics(root: Path) -> pd.DataFrame:
    ...
    root = Path(root)
    candidates = [root] if root.is_file() else [root / "all_metrics.csv", root / "metrics.csv"]
    ...
    for path in candidates:
        if path.exists():
            ...
            break
    if not frames:
        for path in sorted(root.glob("network=*/pij=*/metrics.csv")) + sorted(root.glob("[0-9]*_*/metrics.csv")):
...
def load_ablation_metrics(roots: Iterable[Path], roster: dict[str, tuple[str, str]]) -> pd.DataFrame:
    """Read the numbered method folders without conflating shared PIJ names."""

    frames: list[pd.DataFrame] = []
    for root_value in roots:
        root = Path(root_value)
        paths = sorted(root.glob("[0-9]*_*/metrics.csv")) if root.is_dir() else []
        if root.is_dir() and not paths:
            paths = sorted(root.glob("network=*/pij=*/metrics.csv"))
        if not paths:
            paths = [root]
        for path in paths:
            metrics = load_vertical_metrics(path)
            folder_label = re.sub(r"^\d+_", "", path.parent.name) if path.is_file() else ""
            if folder_label in roster:
                label = folder_label
                network, pij = roster[label]
                if set(metrics["network_method"].astype(str)) != {network} or set(metrics["pij_method"].astype(str)) != {pij}:
                    raise ValueError(f"Ablation identity disagrees with {path}: {label}")
                metrics["Method"] = label
            else:
                by_identity = {(network, pij): label for label, (network, pij) in roster.items()}
                labels = [
                    by_identity.get((str(network), str(pij)))
                    for network, pij in zip(metrics["network_method"], metrics["pij_method"])
                ]
                if any(label is None for label in labels):
                    raise ValueError(f"Cannot identify all ablation rows in {path}")
                metrics["Method"] = labels
            source_file = path if path.is_file() else next(
                (candidate for candidate in (root / "all_metrics.csv", root / "metrics.csv") if candidate.is_file()),
                root,
            )
            metrics["source_metrics"] = str(source_file.resolve())
            frames.append(metrics)
    if not frames:
        raise FileNotFoundError("No PIJ ablation metrics were supplied")
    ...
    for _, group in result.groupby(keys, dropna=False):
        ...
        if len(values) > 1 and not np.allclose(values, values[0], atol=1e-8, rtol=1e-8):
            raise ValueError(f"Conflicting duplicate ablation rows: {group[keys].iloc[0].to_dict()}")
    return result.drop_duplicates(keys, keep="first").reset_index(drop=True)
...
def find_domain_map(data_root: Path, layer: str, organ: str, stage: str) -> Path:
    ...
    if not matches:
        ...
        elif layer == "seurat_k10":
            matches = [p for p in matches if "seurat10" in p.name.lower() or "k10" in str(p.parent).lower()]
    if not matches:
...
def _path_time_pair(path: Path) -> str | None:
    for part in reversed(path.parts):
        match = _TIME_PAIR_RE.search(part)
        if match:
...
def _path_scale(path: Path) -> str | None:
    for part in reversed(path.parts):
        if part in {"spot", "seurat_k150", "seurat_k40"}:
            return part
    if re.match(r"heart_\d+(?:\.\d+)?_to_\d+(?:\.\d+)?_k\d+", path.name):
        return "spot"
    return None
...
def discover_coarse_runs(coarse_root: Path, methods: Iterable[str]) -> pd.DataFrame:
    ...
    for summary_path in sorted(root.rglob("summary.json")):
        ...
        if seed is None and config_path.is_file():
            seed = _json(config_path).get("seed")
        row: dict[str, object] = {
            "method": method,
        ...
        if row["K"] is None and config_path.is_file():
            row["K"] = _json(config_path).get("k")
