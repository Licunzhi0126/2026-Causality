...
from .config import AssetConfig, K10_LEVELS
...
def prepare_paper_tables(cfg: AssetConfig, assets: Iterable[str] | None = None) -> dict[str, object]:
    ...
    if "table4" in requested or "table5" in requested:
        if not cfg.multiscale_roots:
            raise ValueError("Optimal input-scale tables require --multiscale-root")
        runs = discover_coarse_roots(cfg.multiscale_roots, (cfg.primary_coarse_method,))
        selected = select_optimal_input_runs(
            runs, method=cfg.primary_coarse_method,
            k_by_scale=cfg.optimal_k_by_scale, seed=int(cfg.coarse_seed),
            time_pairs=cfg.optimal_time_pairs,
        )
        selected.to_csv(audit_dir / "selected_optimal_input_runs.csv", index=False)
        source_paths.update(Path(path) / "summary.json" for path in selected["run_dir"])
        if "table4" in requested:
            frames["table4"] = build_optimal_grid(
                selected, "delta_EI_best_checkpoint", cfg.optimal_time_pairs,
            )
        if "table5" in requested:
            cg_long = evaluate_optimal_runs(selected)
            cg_long.to_csv(tables_dir / "optimal_input_cg_long.csv", index=False)
            for directory in selected["run_dir"]:
                source_paths.update(Path(directory) / name for name in ("S_t.npy", "S_tp.npy", "PIJ_micro_train.npy"))
            frames["table5"] = build_optimal_grid(
...
def render_paper_assets(cfg: AssetConfig, assets: Iterable[str] | None = None) -> dict[str, object]:
    ...
    required_tables = {
        name for name in requested if name in TABLE_FILES
    }
    ...
    table_renderers = {
        ...
        "table4": lambda frame, path: render_metric_grid(
            frame, path, dpi=cfg.dpi, title="Table 4 · Optimal coarse-graining DeltaEI by input scale",
            subtitle="Each value compares the optimized output EI with its own input EI.",
        ),
        "table5": lambda frame, path: render_metric_grid(
            frame, path, dpi=cfg.dpi, title="Table 5 · Optimal coarse-graining ClosureQuality by input scale",
            subtitle="Matched runs and cells with Table 4; low-signal cells are shown as missing.",
        ),
    }
