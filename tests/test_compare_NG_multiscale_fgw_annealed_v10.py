...
def test_v10_node_cost_is_exactly_the_frozen_v5_cost() -> None:
    n_source, n_target, g_source, g_target = _features()
    expected, _ = build_grnanchored_kl_cost(n_source, n_target, g_source, g_target)
    actual, metadata = CompareNGMultiscaleFGWAnnealedV10PijMethod().build_kl_cost(
        n_source,
        n_target,
    ...
    assert metadata["fixed_internal_temperature_schedule"] == list(
        MULTISCALE_TEMPERATURE_SCHEDULE
    )


def test_v10_is_registered_without_rebinding_frozen_baseline() -> None:
    method = get_pij_method("compare_NG_multiscale_fgw_annealed_v10")
    assert isinstance(method, CompareNGMultiscaleFGWAnnealedV10PijMethod)
    assert PIJ_METHOD_REGISTRY["compare_N_kl"] is CompareNKlPijMethod


def test_v10_config_requires_light_cci_grn() -> None:
    TemporalRunConfig(
        network_method="light_cci_grn",
        pij_method="compare_NG_multiscale_fgw_annealed_v10",
...
def test_v10_builds_balanced_kernel_without_leakage_metadata() -> None:
    method = CompareNGMultiscaleFGWAnnealedV10PijMethod()
    n_source, n_target, g_source, g_target = _features()
    source_adjacency = sp.csr_matrix([[0.0, 1.0], [0.5, 0.0]])
    target_adjacency = sp.csr_matrix(
        [[0.0, 1.0, 0.2], [0.3, 0.0, 1.0], [0.8, 0.4, 0.0]]
    )
    ...
    multiscale = diagnostics["multiscale_fgw"]
    assert multiscale["diffusion_steps"] == list(MULTISCALE_DIFFUSION_STEPS)
    assert multiscale["temperature_schedule"] == list(MULTISCALE_TEMPERATURE_SCHEDULE)
    assert multiscale["uses_ei_for_fitting"] is False
    assert multiscale["uses_layer_identity"] is False
    assert multiscale["uses_third_timepoint"] is False


def test_v10_rejects_parameter_drift() -> None:
    method = CompareNGMultiscaleFGWAnnealedV10PijMethod()
    n_source, n_target, g_source, g_target = _features()
